...
namespace OW {
	...
	struct velocity_compo_t {
		union {
			...
			OFF_MEMB(uint64_t, bonedata, 0x8C0);
		};
	};
	struct hero_compo_t {
		union {
	...
	inline std::string GetHeroEngNames(uint64_t HeroID, uint64_t LinkBase) {
		...
		default:
			return skCrypt(u8"Juno").decrypt();
		}
	}
	...
	inline uintptr_t DecryptComponent(uint64_t parent, uint8_t idx)
	{
		__try {
			if (parent) {
				...
				auto dummy = SDK->RPM<_QWORD>(SDK->dwGameBase + 0x38A5E20 + (Key1 >> 0x34));
				auto dummy2 = SDK->RPM<_QWORD>(SDK->dwGameBase + 0x38A5E20 + (Key1 & 0xFFF));
				uint64_t v12 = (unsigned int)v9 | v9 & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)((unsigned int)v9 + __ROL4__(dummy, 1)) << 0x20);
				uint64_t v13 = -(int)v7 & ((unsigned int)Key2 ^ (unsigned int)v12 | ((unsigned int)Key2 ^ (unsigned int)v12 | (Key2 ^ ((unsigned int)v12 | v12 & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)(unsigned int)(v12 - HIDWORD(dummy2)) << 0x20))) & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)(0x65069CBC - ((unsigned int)Key2 ^ (unsigned int)v12)) << 0x20)) & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)(2 * ((unsigned int)Key2 ^ (unsigned int)v12) - __ROL4__(HIDWORD(dummy), 9)) << 0x20));
				if (SDK->RPM<uint8_t>(v13 + 0x10) == idx)
					return v13;
				return 0;
			}
		}
		...
		inline unsigned __int64 __fastcall DecryptVis(unsigned __int64 a1)
		{
			__try {
				...
				v3 = v2 + 0x8;
				v4 = SDK->RPM<_QWORD>(SDK->dwGameBase + offset::VisRead + 8ull * (((uint8_t)a1 + 0x19) & 0x7F)
					+ (((unsigned __int64)(a1 + offset::Vis_Key1) >> 7) & 7)) ^ v2 ^ (a1 + offset::Vis_Key1);
				v6 = (v3 - v2 + 7) >> 3;
				if (v2 > v3)
					v6 = 0i64;
				if (v6 >= 4)
	...
	inline uint64_t DecryptOutline(uint64_t a1)
	{
		...
		v3 = v2 + 0x8;
		v4 = 0i64;
		v5 = SDK->RPM<uintptr_t>(SDK->dwGameBase + offset::OutlineRead + 8ull * (((uint8_t)a1 + 0x4D) & 0x7F)
