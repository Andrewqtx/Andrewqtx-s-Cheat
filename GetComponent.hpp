...
namespace OW {
	...
	inline uintptr_t DecryptComponent(uint64_t Parent, uint8_t index)
	{
		__try {
			if (Parent) {

				uint64_t v1 = Parent;
				uint64_t v2 = (uint64_t)1 << (uint64_t)(index & 0x3F);
	...
	inline uintptr_t GetHeapManager(uint8_t index) {
		uintptr_t v0 = SDK->RPM<uintptr_t>(SDK->dwGameBase + 0x364D400);
		if (v0 != 0) {
			uintptr_t v1 = (__ROL8__(
				__ROR8__(SDK->RPM<_QWORD>(v0 + 0x160) + 0x133A2F0FA05E59FLL, 0x13) + 0x1A0EF5253B3F7062LL,
				0x1F) ^ 0xF5EF035545B5E77DuLL);
			if (v1 != 0) {
