...
namespace OW {
			...
			for (c_entity entity : entities)
			{
				...
				if (Config::draw_info) {
					if (entity.Alive && entity.Team && local_entity.PlayerHealth > 0) {
						...
						if (Config::drawhealth)Render::DrawStrokeText(ImVec2(Vec2_A.X - width / 6, Vec2_A.Y), ImGui::GetColorU32(ImVec4(0.9, 0.9, 1, 1)), (skCrypt(u8"").decrypt() + std::to_string((int)entity.PlayerHealth) + skCrypt(u8"").decrypt()).c_str(), Size);
						if (Config::ult)Render::DrawStrokeText(ImVec2(Vec2_A.X - width / 6, Vec2_A.Y + Size), ImGui::GetColorU32(ImVec4(0.9, 0.9, 1, 1)), (skCrypt(u8"").decrypt() + std::to_string((int)entity.ultimate) + skCrypt(u8"").decrypt()).c_str(), Size);
						if (Config::dist)Render::DrawStrokeText(ImVec2(Vec2_B.X - width / 6, Vec2_B.Y), ImGui::GetColorU32(ImVec4(0.9, 0.9, 1, 1)), (skCrypt(u8"").decrypt() + std::to_string((int)dist) + skCrypt(u8"").decrypt()).c_str(), Size);
						if (Config::name)Render::DrawStrokeText(ImVec2(Vec2_B.X - width / 6, Vec2_B.Y + Size), ImGui::GetColorU32(ImVec4(0.9, 0.9, 1, 1)), (skCrypt(u8"").decrypt() + GetHeroEngNames(entity.HeroID, entity.LinkBase) + skCrypt(u8"").decrypt()).c_str(), Size);
					}
				}
				if (Config::draw_skel) {
		...
		__try {
			...
			while (FindWindowA(skCrypt("TankWindowClass"), NULL))
			{
				...
				if (Config::Menu)
				{
					...
					bool _visible = true;
					if (ImGui::Begin(skCrypt(u8"Andrewqtx's cheat"), &_visible, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoTitleBar)) {
						ImGui::Spacing();
						ImGui::PushFont(ico);
						...
						ImGui::Spacing();
						ImGui::SameLine();
						ImGui::Text(skCrypt("                    Andrewqtx's cheat"));
						ImGui::PopStyleColor();
						ImGui::PopFont();
						ImGui::PushFont(ico);
						ImGui::Spacing();
	...
	inline void configsavenloadthread() {
		...
		if (Config::lastheroid == -2) {
			ImGui::InsertNotification({ ImGuiToastType_Success, 8000, skCrypt(u8"Andrewqtx's cheat Loaded\nWelcome！"), "" });
			Config::lastheroid = 0;
		}
		while (1) {
			if (!Config::Menu) {
