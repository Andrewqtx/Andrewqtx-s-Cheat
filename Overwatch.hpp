...
namespace OW {
			...
			for (c_entity entity : entities)
			{
				...
				if (Config::draw_info) {
					if (entity.Alive && entity.Team && local_entity.PlayerHealth > 0) {
						...
						if (Size < 16) Size = 16;
						if (Size > 20) Size = 20;
						if (Config::drawbattletag) Render::DrawStrokeText(ImVec2(Vec2_A.X - width / 6, Vec2_B.Y - Size), ImGui::GetColorU32(ImVec4(0, 0.9, 1, 1)), ((u8"") + entity.battletag + (u8"")).c_str(), Size);
						...
						if (Config::drawhealth)Render::DrawStrokeText(ImVec2(Vec2_A.X - width / 6, Vec2_A.Y), ImGui::GetColorU32(ImVec4(0.9, 0.9, 1, 1)), ((u8"") + std::to_string((int)entity.PlayerHealth) + (u8"")).c_str(), Size);
						if (Config::ult)Render::DrawStrokeText(ImVec2(Vec2_A.X - width / 6, Vec2_A.Y + Size), ImGui::GetColorU32(ImVec4(0.9, 0.9, 1, 1)), ((u8"") + std::to_string((int)entity.ultimate) + (u8"")).c_str(), Size);
						if (Config::dist)Render::DrawStrokeText(ImVec2(Vec2_B.X - width / 6, Vec2_B.Y), ImGui::GetColorU32(ImVec4(0.9, 0.9, 1, 1)), ((u8"") + std::to_string((int)dist) + (u8"")).c_str(), Size);
						if (Config::name)Render::DrawStrokeText(ImVec2(Vec2_B.X - width / 6, Vec2_B.Y + Size), ImGui::GetColorU32(ImVec4(0.9, 0.9, 1, 1)), ((u8"") + GetHeroEngNames(entity.HeroID, entity.LinkBase) + (u8"")).c_str(), Size);
					}
				} 
				if (Config::draw_skel) {
	...
	inline void configsavenloadthread() {
		...
		if (Config::lastheroid == -2) {
			ImGui::InsertNotification({ ImGuiToastType_Success, 8000, (u8"Andrewqtx's cheat Loaded\nWelcome！"), "" });
			Config::lastheroid = 0;
		}
		while (1) {
			if (!Config::Menu) {
